 ...
 *
 */
package com.support.android_watermark;
...
import com.support.android_watermark.bean.WatermarkGravity;
import com.support.android_watermark.bean.WatermarkImage;
import com.support.android_watermark.bean.WatermarkText;
import com.support.android_watermark.listener.BuildFinishListener;
import com.support.android_watermark.task.FDWatermarkTask;
import com.support.android_watermark.task.LSBWatermarkTask;
...
import static com.support.android_watermark.utils.BitmapUtils.textAsBitmap;
        ...
        if (watermarkImg != null && backgroundImg != null) {
            if (isInvisible) {
                ...
                if (isTileMode) {
                    ...
                    if (watermarkImg.getWatermarkGravity().getGravity() == WatermarkGravity.NONE) {
                        watermarkCanvas.drawBitmap(scaledWMBitmap,
                                ...
                                watermarkPaint);
                    } else if (watermarkImg.getWatermarkGravity().getGravity() == WatermarkGravity.BOTTOM_RIGHT) {
                        watermarkCanvas.drawBitmap(scaledWMBitmap,
                                backgroundImg.getWidth() - scaledWMBitmap.getWidth() - watermarkImg.getWatermarkGravity().getMargin(),
                                backgroundImg.getHeight() - scaledWMBitmap.getHeight() - watermarkImg.getWatermarkGravity().getMargin(),
                                watermarkPaint);
                    } else if (watermarkImg.getWatermarkGravity().getGravity() == WatermarkGravity.BOTTOM_LEFT) {
                        watermarkCanvas.drawBitmap(scaledWMBitmap,
                                watermarkImg.getWatermarkGravity().getMargin(),
                                backgroundImg.getHeight() - scaledWMBitmap.getHeight() - watermarkImg.getWatermarkGravity().getMargin(),
                                watermarkPaint);
                    }else if (watermarkImg.getWatermarkGravity().getGravity() == WatermarkGravity.TOP_LEFT) {
                        watermarkCanvas.drawBitmap(scaledWMBitmap,
                                watermarkImg.getWatermarkGravity().getMargin(),
                                watermarkImg.getWatermarkGravity().getMargin(),
                                watermarkPaint);
                    }else if (watermarkImg.getWatermarkGravity().getGravity() == WatermarkGravity.TOP_RIGHT) {
                        watermarkCanvas.drawBitmap(scaledWMBitmap,
                                backgroundImg.getWidth() - scaledWMBitmap.getWidth() - watermarkImg.getWatermarkGravity().getMargin(),
                                watermarkImg.getWatermarkGravity().getMargin(),
                                watermarkPaint);
