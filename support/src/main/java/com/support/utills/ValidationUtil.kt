...
object ValidationUtil {
    ...
    fun isValidName(context: Context, txtInput: String?): Boolean {
        when {
            isNullOrEmpty(txtInput) -> showToast(context, "Please enter Name first.")
            txtInput!!.length < 4 -> showToast(context, "Name length should not be less than 4 characters")
            txtInput.length > 30 -> showToast(context, "Name length should not be greater than 30 characters")
            else -> return true
        }
        return false
    }
